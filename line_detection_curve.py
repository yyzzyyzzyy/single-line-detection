...
# 画霍夫变换直线
def draw_houghline(image,lines,color=[0,255,0], thickness=2):
    for line in lines:
        for x1, y1, x2, y2 in line:
            fit = np.polyfit((x1, x2), (y1, y2), 1)  # 拟合成直线 一阶
            slope = fit[0]  # 斜率

            if 1 < np.absolute(slope) <= 12:
                cv.line(image, (x1, y1), (x2, y2), color, thickness)

# 画车道线
def draw_lines(image, lines, midx=720, color=[255,0,0], thickness=2):
...
# hough变换后的车道线拟合（曲线）
def fit_poly(img, lines, midx = 640):
...
# 两个一维数组画线
def draw(image, X, Y, color=[255,0,0], thickness=2):
...
def eucliDist(Curve, lanes, h_samples):
    ...
    for lane in lanes:
        ...
        for x1, y1, x2, y2 in Curve:
            ...
            X_real = np.poly1d([fit[0], fit[1]])
            X_real = X_real(Y) # 将x值带入方程求y
            # 计算俩车道线与左右车道标签的距离
            print("欧式距离:", np.sqrt(sum(np.power((X - X_real), 2))))

            # Frechet距离
            value_similarity(X, X_real, Y)
            # # 拟合优度 针对回归曲线
...
# 亮度划分函数   l通道
def hlsLSelect(img, thresh=(120,255)):# 220,255
...
# lab蓝黄通道划分函数
def labBSelect(img,thresh=(230,255)):  # 215,255
...
def process_image(image):
    ...
    # # 预处理
    # hlsL_binary = hlsLSelect(persprctie_image)
    # labB_binary = labBSelect(persprctie_image)
    # combined_binary = np.zeros_like(persprctie_image)
    # combined_binary[(hlsL_binary == 255) | (labB_binary == 255)] = 255
    # cv.imshow('img_binary', combined_binary)

    # 1.灰度图转换
    gray = grayscale(perspective_image)
    # cv.imshow('gray', gray)
    ...
    # 4.生成Mask掩模
    # vertices = np.array([[(0, imshape[0]), (9 * imshape[1] / 20, 11 * imshape[0] / 17),(12 * imshape[1] / 20, 11 * imshape[0] / 17), (imshape[1], imshape[0])]], dtype=np.int32)
    # vertices = np.array([[(0 + 20, imshape[0] - 60), (600, 450), (720, 450), (imshape[1] - 150image.shape imshape[0] - 60)]])
    # vertices = np.array([[(2 * image.shape[1] / 20, image.shape[0]), (9 * image.shape[1] / 20, 6 * image.shape[0] / 18),
    #             (12 * image.shape[1] / 20, 6 * image.shape[0] / 18), (image.shape[1], 15 * image.shape[0] / 17), (image.shape[1], image.shape[0])]], dtype=np.int32)
    vertices = np.array([[(1 * image.shape[1] / 3, image.shape[0]), (1 * image.shape[1] / 3, 2 * image.shape[0] / 7),
                (7 * image.shape[1] / 18, 2 * image.shape[0] / 7), (6 * image.shape[1] / 11, image.shape[0])]], dtype=np.int32)
    ...
    hof_threshold = 3  # 霍夫平面累加阈值threshold
    min_line_len = 3  # 线段最小长度
    ...
    # # 测试
    midx = int(7 * image.shape[1] / 18)  # 画中线(参考线)
    # draw_houghline(perspective_image, lines)  # 透视变换后的画中的ROI上，画满足条件的HOUGH变换的直线
    # cv.line(perspective_image, (midx, int(image.shape[0])), (midx, 0), color=[0, 255, 0], thickness=1)

    # draw_correctline(image, dataset.lanes, dataset.h_samples)  # 标签车道线
    # cv.imshow('test_image', image)
    # draw_lines(perspective_image, lines, midx)
    fit_poly(perspective_image, lines, midx)  # 在透视变换的图中画车道线 用于观察拟合效果
    cv.imshow("perspective_image", perspective_image)

    # 6.绘制车道线线段
    # Curve = draw_lines(line_image, lines, midx, thickness=10)
    # draw_lines(line_image, lines, midx)
    fit_poly(line_image, lines, midx)  # 只画车道线 用于透视逆变换 图像融合
...
if __name__ == '__main__':
    ...
    image = cv.imread('D:/Pycharm/coding/Dataset/7right.jpg')
